...
(function($) {
	...
	*/
	...
	function animateBox() {
        ...
        if (width + paddingWidth > $(window).width() || height + paddingHeight + 32 > $(window).height()){
            var new_width = $(window).width() - paddingWidth - 20,
                new_height =  new_width * ratio;

            if (new_height + paddingHeight + 32 > $(window).height()){
                new_height = $(window).height() - paddingHeight - 32; //20;
		...
		totalHeight = centerHeight + paddingHeight + 0; // + 0;
		var top = Math.max(0, middle - (totalHeight / 2) - 16);
